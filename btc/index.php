...
<?php
$api_url = 'https://min-api.cryptocompare.com/data/pricemulti?fsyms=IOT,ADA,ARK,NEO,GAS,DGB,XEM,XLM,XRP,ETH,LTC,DASH,XMR,ZEC,DOGE&tsyms=BTC';
$cryptocurrency = file_get_contents($api_url);
$json=json_decode($cryptocurrency,true);
 
foreach($json as $cc => $value) {
   echo '<div class="ccrow"><div class="col-xs-3 ccx"><span>'. $cc.'</span></div><div class="col-xs-9 cct"><span>'. $value['BTC'] .'</span></div></div>';
}
