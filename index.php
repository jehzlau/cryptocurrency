...
<?php
$api_url = 'https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,IOT,ADA,ARK,NEO,GAS,BCN,DGB,XEM,XLM,XRP,ETH,LTC,DASH,XMR,ZEC,DOGE&tsyms=PHP';
$cryptocurrency = file_get_contents($api_url);
$json=json_decode($cryptocurrency,true);
 
foreach($json as $cc => $value) {
   echo '<div class="ccrow"><div class="col-xs-3 ccx"><span>'. $cc.'</span></div><div class="col-xs-9 cct"><span>&#8369; '. number_format($value['PHP'], 2, '.', ',') .'</span></div></div>';
}
